#!/bin/bash
# =========================
# Argo + VMess + WS 安装脚本 (原生增强版)
# 基于你提供的最原始版本，仅修复 hu 命令、固定隧道显示和 v6 适配
# =========================
...
# 生成节点信息 (已修复: 防止 UUID 丢失 + 支持固定隧道读取)
get_info() {
    # 修复：防止变量丢失
...
if [ -s "$LOCAL_SCRIPT" ]; then
    ...
    echo -e "\\033[1;33m脚本丢失，尝试从 GitHub 重新下载...\\033[0m"
    mkdir -p "$work_dir"
    curl -sLo "$LOCAL_SCRIPT" "$GITHUB_URL"
    chmod +x "$LOCAL_SCRIPT"
    if [ -s "$LOCAL_SCRIPT" ]; then
        ...
        echo -e "\\033[1;91m下载失败，请检查网络\\033[0m"
    fi
fi
EOF
    chmod +x "/usr/bin/hu"
...
# Argo固定隧道配置 (已修复：Token v6 写入 yml 供读取 + 自动刷新)
setup_argo_fixed() {
    clear
    yellow "\n固定隧道可为json或token，固定隧道端口为${vmess_port}\n"
...
EOF
        ...
        # 修复：Token模式也生成一个伪造的 yml，仅仅为了让 get_info 能读取到域名
